...
class Generator:
    ...
    def _get_question_prompts(
        ...
        messages = []
        if (
            self._jail_break_type in ["before", "all"]
            and "before" in self._jail_break_dict
        ):
            messages.append(
                llm_entities.Message(
                    role="system", content=self._jail_break_dict["before"]
                )
            )
        if system_prompt:
            messages.append(llm_entities.Message(role="system", content=system_prompt))
        if (
            self._jail_break_type in ["after", "all"]
            and "after" in self._jail_break_dict
        ):
            messages.append(
                llm_entities.Message(
                    role="system", content=self._jail_break_dict["after"]
                )
            )
    ...
    def _get_image_prompts(
        self, content_list: list[llm_entities.ContentElement], system_prompt: str = None
    ) -> typing.List[llm_entities.Message]:
        messages = []
        if system_prompt:
            messages.append(llm_entities.Message(role="system", content=system_prompt))
    ...
    async def select_from_list(
        self, question: str, options: list, system_prompt: str = None
    ) -> str:
        ...
        messages = self._get_question_prompts(
            prompt, output_format="text", system_prompt=system_prompt
        )
    ...
    async def return_list(
        self, question: str, system_prompt: str = None, generate_tags: bool = False
    ) -> list:
        model_info = await self.ap.model_mgr.get_model_by_name(
            self.ap.provider_cfg.data["model"]
        )
        ...
        messages = self._get_question_prompts(
            prompt, output_format="JSON list", system_prompt=system_prompt
        )
    ...
    async def return_json(self, question: str, system_prompt: str = None) -> list:
        ...
        messages = self._get_question_prompts(
            question, output_format="JSON", system_prompt=system_prompt
        )
    ...
    async def return_number(self, question: str, system_prompt: str = None) -> int:
        ...
        messages = self._get_question_prompts(
            prompt, output_format="number", system_prompt=system_prompt
        )
    ...
    async def return_string(self, question: str, system_prompt: str = None) -> str:
        ...
        messages = self._get_question_prompts(
            question, output_format="text", system_prompt=system_prompt
        )
    ...
    def clean_response(self, response: str) -> str:
        if self._speakers:
            ...
            speakers_pattern = "|".join(
                [re.escape(speaker) for speaker in self._speakers]
            )
    ...
    def _parse_json_list(self, response: str, generate_tags: bool = False) -> list:
        try:
            ...
            json_str = re.sub(
                r"[^\u4e00-\u9fa5A-Za-z\s\[\],\":]", " ", json_str
            ).strip()  # Remove invalid characters and strip
    ...
    def get_content_str_without_timestamp(
        self, message: llm_entities.Message | str
    ) -> str:
        message_content = ""
        ...
        message_content = re.sub(
            r"\[\d{2}年\d{2}月\d{2}日(上午|下午)?\d{2}时\d{2}分\]", "", message_content
        )

        return message_content.strip()

    def messages_to_readable_str(
        self, messages: typing.List[llm_entities.Message]
    ) -> str:
    ...
    def set_jail_break(self, jail_break_type: str, user_name: str):
        ...
        if jail_break_type == "all":
            ...
            for type_name in ["before", "after", "end"]:
                ...
                if os.path.exists(filepath):
                    with open(filepath, "r", encoding="utf-8") as f:
                        self._jail_break_dict[type_name] = f.read().replace(
                            "{{user}}", user_name
                        )
        else:
            ...
            if os.path.exists(filepath):
                with open(filepath, "r", encoding="utf-8") as f:
                    self._jail_break_dict[jail_break_type] = f.read().replace(
                        "{{user}}", user_name
                    )
